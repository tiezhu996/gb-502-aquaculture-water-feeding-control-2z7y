...
- 执行反馈：仅允许已批准计划进入执行，记录实际量、现场溶解氧与反馈；异常中止按实际已投喂量结算并支持差额补排。
...
| `PATCH` | `/api/executions/:id/abort` | 异常中止（中止原因 + 已投喂量） |
| `POST` | `/api/executions/:id/reschedule` | 从已中止记录按当日差额发起补排 |
...
- 待执行或执行中的记录可填写中止原因与已投喂量后异常中止（`aborted`）；中止记录按实际量计入当日累计，未投喂部分释放且不再占用原计划量。
- 补排只能从已中止记录发起，补排时间须在原计划当日，补排量不得超过计划日量的未达差额；余额不足拒绝、余额为零不能补排。补排生成与原中止记录关联的新安排，原记录保留中止状态与补排关系；删除待执行的补排安排会解除关联，原中止记录可重新补排。
